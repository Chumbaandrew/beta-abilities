...
async function loadQuestions() {
    ...
    //get the container
    const questionContainer = document.getElementById("HearingContainer");

    //filter only adaptation(visually impaired) questions
    const adaptationsQuestions =
        questions.filter(question =>
            question.section === "Hearing"
        );
...
async function calculateScore() {
    ...
    try {
        ...
        await setDoc(
            doc(db, "institutions", user.uid, "assessments", "auditory"),
            {
                score: totalScore,
        ...
        alert("Auditory assessment saved!");
